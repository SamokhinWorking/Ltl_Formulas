...
import ltlgenCondition.formulas.Verifiable;

import java.io.File;
import java.io.IOException;
import java.util.*;

public class DataConditionStatistics extends Statistics {
    ...
    private String individualToString(boolean forHumans, GPIndividual individual, Set<String> sh) {
        String s;
        if (forHumans) {
            s = individual.trees[0].child.toStringForHumans() + "\n";
        } else {
            s =  ((Verifiable) individual.trees[0].child).toStringForVerifier() + "\n";
        }
        if (sh.contains(s)) {
            return "";
        }
